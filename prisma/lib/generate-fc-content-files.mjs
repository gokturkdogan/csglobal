 ...
 * prisma/data/fc-translations/_batch1-translations/{slug}.json
 */
import fs from "fs";
...
const TRANSLATIONS_DIR = path.join(
  process.cwd(),
  "prisma/data/fc-translations/_batch1-translations",
);
...
function main() {
  ...
  for (const file of files.sort()) {
    ...
    fs.writeFileSync(path.join(OUTPUT_DIR, `${slug}.ts`), out);
