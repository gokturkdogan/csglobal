...
/** Gövde metninde geçen genel kelimeler (Bilgilendirme, Euroasia Global) kullanılmaz. */
...
/** EAGVS HTTP isteği zaman aşımı (yavaş veya yanıt vermeyen sayfalar takılmayı önler). */
export const EAGVS_FETCH_TIMEOUT_MS = 45_000;

export async function fetchEagvsHtml(url: string): Promise<string> {
  const res = await fetch(url, {
    headers: {
      Accept: "text/html,application/xhtml+xml",
      "User-Agent": "CSGlobal-EAGVS-Sync/1.0",
    },
    signal: AbortSignal.timeout(EAGVS_FETCH_TIMEOUT_MS),
  });
  if (!res.ok) {
...
export async function scrapeEagvsPage(
  url: string,
  htmlOverride?: string,
): Promise<EagvsScrapedPage> {
  const html = htmlOverride ?? await fetchEagvsHtml(url);
